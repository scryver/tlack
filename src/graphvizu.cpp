...
print_graph_line(GraphState *state, char *fmt, ...)
{
    if (state->indent)
    {
        ...
        fprintf(stream2stdfile(state->output), "%s", indent);
    }
    
    va_list args;
    va_start(args, fmt);
    vfprintf(stream2stdfile(state->output), fmt, args);
    va_end(args);
    fputc('\n', stream2stdfile(state->output));
}
...
graph_variable(GraphState *state, Variable *var)
{
    ...
    {
        snprintf(buf, sizeof(buf), "%.*s_%d", STR_FMT(var->id->name), state->index);
        label = var->id->name;
        result = str_intern_c(&gTokenInterns, buf);
    }
    else
    {
        i_expect(var->kind == VARIABLE_CONSTANT);
        snprintf(buf, sizeof(buf), "%d", var->constant->value);
        label = str_intern_c(&gTokenInterns, buf);
...
graph_assignment(GraphState *state, Assignment *assign)
{
    ...
    char buf[128];
    snprintf(buf, sizeof(buf), "%.*s_%d", STR_FMT(name), state->index);
    String result = str_intern_c(&gTokenInterns, buf);
...
{
    GraphState state = {0};
    state.output.file = open_file(string(string_length(fileName), fileName), FileOpen_Write);
         ...
         ++statementIndex)
    {
        ...
        String next = str_intern_c(&gTokenInterns, buf);
...
save_graph(char *fileName, u32 registers, u32 opCodeCount, OpCode *opCodes)
{
    ...
    graphicStream.file = open_file(string(string_length(fileName), fileName), FileOpen_Write);
    
    fprintf(stream2stdfile(graphicStream), "digraph opcodes {\n");
    fprintf(stream2stdfile(graphicStream), "  ranksep=.75;\n");
    
    for (u32 regIdx = 0; regIdx < registers; ++regIdx)
    {
        fprintf(stream2stdfile(graphicStream), "    mem%d [shape=record, label=\"{ <in>in | mem[%d] | {<a>a|<b>b} }\"];\n", regIdx, regIdx);
    }
    ...
    for (u32 opIdx = 0; opIdx < opCodeCount; ++opIdx)
    {
        ...
        fprintf(stream2stdfile(graphicStream), "  subgraph cluster%d {\n", opIdx);
        ...
        {
            case Select_Zero: {
                //fprintf(stream2stdfile(graphicStream), "    ZERO%03d -> ioOut%03d\n",
                //opIdx, opIdx + 1);
            } break;
            case Select_MemoryA: {
                fprintf(stream2stdfile(graphicStream), "    memA%03d -> ioOut%03d\n",
                        opIdx, opIdx);
                layerHas |= LAYER_HAS_MEMA | LAYER_HAS_IOUT;
            } break;
            case Select_MemoryB: {
                fprintf(stream2stdfile(graphicStream), "    memB%03d -> ioOut%03d\n",
                        opIdx, opIdx);
                layerHas |= LAYER_HAS_MEMB | LAYER_HAS_IOUT;
            } break;
            case Select_Immediate: {
                fprintf(stream2stdfile(graphicStream), "    imm%03d -> ioOut%03d\n",
                        opIdx, opIdx);
                layerHas |= LAYER_HAS_IMM | LAYER_HAS_IOUT;
            } break;
            case Select_IO: {
                fprintf(stream2stdfile(graphicStream), "    ioIn%03d -> ioOut%03d\n",
                        opIdx, opIdx);
                layerHas |= LAYER_HAS_IOIN | LAYER_HAS_IOUT;
            } break;
            case Select_Alu: {
                fprintf(stream2stdfile(graphicStream), "    alu%03d:out -> ioOut%03d\n",
                        opIdx - 1, opIdx);
        ...
        {
            case Select_Zero: {
                //fprintf(stream2stdfile(graphicStream), "    ZERO%03d -> memIn%03d\n",
                //opIdx, opIdx);
            } break;
            case Select_MemoryA: {
                fprintf(stream2stdfile(graphicStream), "    memA%03d -> memIn%03d\n",
                        opIdx, opIdx);
                layerHas |= LAYER_HAS_MEMA | LAYER_HAS_MEMI;
            } break;
            case Select_MemoryB: {
                fprintf(stream2stdfile(graphicStream), "    memB%03d -> memIn%03d\n",
                        opIdx, opIdx);
                layerHas |= LAYER_HAS_MEMB | LAYER_HAS_MEMI;
            } break;
            case Select_Immediate: {
                fprintf(stream2stdfile(graphicStream), "    imm%03d -> memIn%03d\n",
                        opIdx, opIdx);
                layerHas |= LAYER_HAS_IMM | LAYER_HAS_MEMI;
            } break;
            case Select_IO: {
                fprintf(stream2stdfile(graphicStream), "    ioIn%03d -> memIn%03d\n",
                        opIdx, opIdx);
                layerHas |= LAYER_HAS_IOIN | LAYER_HAS_MEMI;
            } break;
            case Select_Alu: {
                fprintf(stream2stdfile(graphicStream), "    alu%03d:out -> memIn%03d\n",
                        opIdx - 1, opIdx);
        ...
        {
            case Select_Zero: {
                //fprintf(stream2stdfile(graphicStream), "    ZERO%03d -> aluA%03d\n",
                //opIdx - 1, opIdx);
            } break;
            case Select_MemoryA: {
                fprintf(stream2stdfile(graphicStream), "    memA%03d -> alu%03d:a\n",
                        opIdx, opIdx);
                layerHas |= LAYER_HAS_MEMA;
            } break;
            case Select_MemoryB: {
                fprintf(stream2stdfile(graphicStream), "    memB%03d -> alu%03d:a\n",
                        opIdx, opIdx);
                layerHas |= LAYER_HAS_MEMB;
            } break;
            case Select_Immediate: {
                fprintf(stream2stdfile(graphicStream), "    imm%03d -> alu%03d:a\n",
                        opIdx, opIdx);
                layerHas |= LAYER_HAS_IMM;
            } break;
            case Select_IO: {
                fprintf(stream2stdfile(graphicStream), "    ioIn%03d -> alu%03d:a\n",
                        opIdx, opIdx);
                layerHas |= LAYER_HAS_IOIN;
            } break;
            case Select_Alu: {
                fprintf(stream2stdfile(graphicStream), "    alu%03d:out -> alu%03d:a\n",
                        opIdx - 1, opIdx);
            } break;
            INVALID_DEFAULT_CASE;
        }
        ...
        {
            case Select_Zero: {
                //fprintf(stream2stdfile(graphicStream), "    ZERO%03d -> aluB%03d\n",
                //opIdx - 1, opIdx);
            } break;
            case Select_MemoryA: {
                fprintf(stream2stdfile(graphicStream), "    memA%03d -> alu%03d:b\n",
                        opIdx, opIdx);
                layerHas |= LAYER_HAS_MEMA;
            } break;
            case Select_MemoryB: {
                fprintf(stream2stdfile(graphicStream), "    memB%03d -> alu%03d:b\n",
                        opIdx, opIdx);
                layerHas |= LAYER_HAS_MEMB;
            } break;
            case Select_Immediate: {
                fprintf(stream2stdfile(graphicStream), "    imm%03d -> alu%03d:b\n",
                        opIdx, opIdx);
                layerHas |= LAYER_HAS_IMM;
            } break;
            case Select_IO: {
                fprintf(stream2stdfile(graphicStream), "    ioIn%03d -> alu%03d:b\n",
                        opIdx, opIdx);
                layerHas |= LAYER_HAS_IOIN;
            } break;
            case Select_Alu: {
                fprintf(stream2stdfile(graphicStream), "    alu%03d:out -> alu%03d:b\n",
                        opIdx - 1, opIdx);
            } break;
            INVALID_DEFAULT_CASE;
        }
        ...
        if (opCode.selectAluA || opCode.selectAluB || (layerHas & LAYER_HAS_ALU))
        {
            ...
            fprintf(stream2stdfile(graphicStream), "    alu%03d [shape=record, label=\"{ {<a>a|<b>b} | %s[%03d] | <out>out}\"];\n", opIdx, aluOp, opIdx);
        ...
        {
            fprintf(stream2stdfile(graphicStream), "    mem%d:a -> loadA%03d -> memA%03d\n",
                    opCode.memoryAddrA, opIdx, opIdx + 1);
            nextLayerHas |= LAYER_HAS_MEMA;
        }
        if (opCode.memoryReadB)
        {
            fprintf(stream2stdfile(graphicStream), "    mem%d:b -> loadB%03d -> memB%03d\n",
                    opCode.memoryAddrB, opIdx, opIdx + 1);
            nextLayerHas |= LAYER_HAS_MEMB;
        }
        if (opCode.memoryWrite)
        {
            fprintf(stream2stdfile(graphicStream), "    memIn%03d -> write%03d -> mem%d:in\n",
                    opIdx, opIdx + 1, opCode.memoryAddrA);
            nextLayerHas |= LAYER_HAS_MEMW;
        }
        
        
        fprintf(stream2stdfile(graphicStream), "      { rank = same; ");
        if (layerHas & LAYER_HAS_MEMA)
        {
            fprintf(stream2stdfile(graphicStream), "memA%03d; ", opIdx);
        }
        if (layerHas & LAYER_HAS_MEMB)
        {
            fprintf(stream2stdfile(graphicStream), "memB%03d; ", opIdx);
        }
        if (layerHas & LAYER_HAS_MEMI)
        {
            fprintf(stream2stdfile(graphicStream), "memIn%03d; ", opIdx);
        }
        
        if (nextLayerHas & LAYER_HAS_MEMA)
        {
            fprintf(stream2stdfile(graphicStream), "loadA%03d; ", opIdx);
        }
        if (nextLayerHas & LAYER_HAS_MEMB)
        {
            fprintf(stream2stdfile(graphicStream), "loadB%03d; ", opIdx);
        }
        if (layerHas & LAYER_HAS_MEMW)
        {
            fprintf(stream2stdfile(graphicStream), "write%03d; ", opIdx);
        }
        if (layerHas & LAYER_HAS_IMM)
        {
            fprintf(stream2stdfile(graphicStream), "imm%03d; ", opIdx);
        }
        if (layerHas & LAYER_HAS_IOIN)
        {
            fprintf(stream2stdfile(graphicStream), "ioIn%03d; ", opIdx);
        }
        if (layerHas & LAYER_HAS_IOUT)
        {
            fprintf(stream2stdfile(graphicStream), "ioOut%03d; ", opIdx);
        }
        if (layerHas & LAYER_HAS_ALU)
        {
            //fprintf(stream2stdfile(graphicStream), "alu%03d; ", opIdx);
        }
        fprintf(stream2stdfile(graphicStream), "};\n");
        
        fprintf(stream2stdfile(graphicStream), "  }\n");
    }
    fprintf(stream2stdfile(graphicStream), "}\n");
    
    close_file(&graphicStream.file);
}
...
print_selection(FileStream stream, char *name, u32 index, enum Selection select)
{
    ...
    fprintf(stream2stdfile(stream), "    sel%s%d [shape=record, label=\"%s\"];\n", name, index, switchIn);
    fprintf(stream2stdfile(stream), "    out%s%d [shape=record, label=\"%s Out\"];\n", name, index, name);
    
    fprintf(stream2stdfile(stream), "    sel%s%d:0 -> out%s%d%s;\n", name, index, name, index,
            (select == Select_Zero) ? "" : " [style=dotted]");
    fprintf(stream2stdfile(stream), "    sel%s%d:a -> out%s%d%s;\n", name, index, name, index,
            (select == Select_MemoryA) ? "" : " [style=dotted]");
    fprintf(stream2stdfile(stream), "    sel%s%d:b -> out%s%d%s;\n", name, index, name, index,
            (select == Select_MemoryB) ? "" : " [style=dotted]");
    fprintf(stream2stdfile(stream), "    sel%s%d:imm -> out%s%d%s;\n", name, index, name, index,
            (select == Select_Immediate) ? "" : " [style=dotted]");
    fprintf(stream2stdfile(stream), "    sel%s%d:io -> out%s%d%s;\n", name, index, name, index,
            (select == Select_IO) ? "" : " [style=dotted]");
    fprintf(stream2stdfile(stream), "    sel%s%d:alu -> out%s%d%s;\n", name, index, name, index,
            (select == Select_Alu) ? "" : " [style=dotted]");
    
    fprintf(stream2stdfile(stream), "    mem%d:a:e -> sel%s%d:a%s;\n", index - 1, name, index,
            (select == Select_MemoryA) ? "" : " [style=dotted]");
    fprintf(stream2stdfile(stream), "    mem%d:b:e -> sel%s%d:b%s;\n", index - 1, name, index,
            (select == Select_MemoryB) ? "" : " [style=dotted]");
    fprintf(stream2stdfile(stream), "    imm%d:e -> sel%s%d:imm%s;\n", index, name, index,
            (select == Select_Immediate) ? "" : " [style=dotted]");
    fprintf(stream2stdfile(stream), "    ioIn%d:e -> sel%s%d:io%s;\n", index, name, index,
            (select == Select_IO) ? "" : " [style=dotted]");
    fprintf(stream2stdfile(stream), "    alu%d:out -> sel%s%d:alu%s;\n", index - 1, name, index,
            (select == Select_Alu) ? "" : " [style=dotted]");
}
...
test_graph(char *fileName, u32 registers, u32 opCodeCount, OpCode *opCodes, b32 minimize)
{
    ...
    fprintf(stream2stdfile(graphicStream), "digraph testing {\n");
    fprintf(stream2stdfile(graphicStream), "    rankdir=LR;\n    ranksep=\"1\";\n\n");
    
#if 0    
    fprintf(stream2stdfile(graphicStream), "    imm0 [shape=record, label=\"0\"];\n");
    fprintf(stream2stdfile(graphicStream), "    ioIn0 [shape=record, label=\"IO In\"];\n");
#endif
    fprintf(stream2stdfile(graphicStream),"    mem0 [shape=record, label=\"{{mem[0]|mem[0]}|{<a>a|<b>b}}\"];\n");
    fprintf(stream2stdfile(graphicStream),"    alu0 [shape=record, label=\"{alu|<out>out}\"];\n");
    ...
    for (u32 opIdx = 0; opIdx < opCodeCount; ++opIdx)
    {
        ...
        if (!minimize || opCode.memoryWrite)
        {
            ...
            fprintf(stream2stdfile(graphicStream), "    outMem%d:e -> mem%d:in%s;\n", graphIdx, graphIdx,
        ...
        if (!minimize || (opCode.aluOperation != Alu_Noop) || (opCode.selectAluA))
        {
            ...
            fprintf(stream2stdfile(graphicStream), "    outAluA%d:e -> alu%d:a:w;\n", graphIdx, graphIdx);
        }
        if (!minimize || (opCode.aluOperation != Alu_Noop))
        {
            print_selection(graphicStream, "AluB", graphIdx, opCode.selectAluB);
            fprintf(stream2stdfile(graphicStream), "    outAluB%d:e -> alu%d:b:w;\n", graphIdx, graphIdx);
        }
    }
    
    fprintf(stream2stdfile(graphicStream), "}\n");
    
    fclose(stream2stdfile(graphicStream));
}
